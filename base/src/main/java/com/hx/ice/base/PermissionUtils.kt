...
object PermissionUtils {
    ...
    fun sdCardPermission(context: Activity, callback: () -> Unit, denied: () -> Unit = {}) {
        permissionCheck(
            context,
    ...
    fun showPermissions(mContext: Activity, permissionName: String, requestCode: Int, callback: (Boolean) -> Unit) {
        showPermissionsTextDialog(mContext, "需要手动开启${permissionName}权限才能使用", requestCode, true, callback)
    }

    private var dialog: AlertDialog? = null
    //权限设置提示框
    fun showPermissionsTextDialog(mContext: Activity, text: String, requestCode: Int, isJumpSetting: Boolean, callback: (Boolean) -> Unit) {
        if (dialog != null)
            return
        dialog = android.app.AlertDialog.Builder(mContext, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT).create().apply {
            setCancelable(false)
            setTitle("提示")
            setMessage(text)
            ...
            setButton(DialogInterface.BUTTON_POSITIVE, "确认") { _, _ ->
                callback(false)
                if (isJumpSetting)
    ...
    fun gotoPerMissionSetting(context: Activity, requestCode: Int) {
        ...
        try {
            ...
            when (Build.BRAND.toLowerCase()) {
                "vivo" -> intent = context.packageManager.getLaunchIntentForPackage("com.iqoo.secure")
                    ?: findSettingUi(intent, context.packageName)
                "huawei" -> {
                ...
                "xiaomi" -> {
                    ...
                    when (getMiuiVersion().toLowerCase()) {
                        ...
                        else -> findSettingUi(intent, context.packageName)
                    }
                }
                else -> findSettingUi(intent, context.packageName)
            }
    ...
    private fun findSettingUi(intent: Intent, pkgName: String): Intent {
